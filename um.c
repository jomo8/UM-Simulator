...
void segmented_load(UM_T um, uint32_t ra, uint32_t rb, uint32_t rc)
{
        ...
        uint32_t rb_val = um->registers[rb];
        uint32_t rc_val = um->registers[rc];


        um->registers[ra] = memory_get(um->mem, rb_val, rc_val);
...
void segmented_store(UM_T um, uint32_t ra, uint32_t rb, uint32_t rc)
{
        ...
        uint32_t ra_val = um->registers[ra];
        uint32_t rb_val = um->registers[rb];

        memory_put(um->mem, ra_val, rb_val, um->registers[rc]);
}
